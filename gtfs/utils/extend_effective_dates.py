...
def extend_feed(feed_path, effective_days):
    ...
    try:
        with zipfile.ZipFile(feed_path, "r") as feedzip:
            if "calendar.txt" not in feedzip.namelist():
                LOG.warn(
                    "Feed %s has no calendar.txt; cannot extend effective date range.",
                    file_name,
                )
                return
    ...
    if cal:
        ...
        with zipfile.ZipFile(
            os.path.join(os.path.dirname(feed_path), file_name[:-4] + "_extended.zip"),
            "w",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Full path to GTFS directory (default: ./%s/)" % DOWNLOAD_DIRECTORY,
    )
    parser.add_argument(
        "--extend-days",
        "-e",
        ...
        help="Extend GTFS this many days into past and future (default: %s)" % EFFECTIVE_DAYS,
    )
    parser.add_argument(
        "--feeds",
        "-f",
